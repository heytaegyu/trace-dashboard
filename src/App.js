...
const Tip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 8, padding: "8px 14px", boxShadow: "0 4px 12px #00000010" }}>
...
const Card = ({ label, value, sub, color }) => (
  <div style={{ background: "#fff", borderRadius: 12, padding: "18px 20px", borderTop: `3px solid ${color}`, boxShadow: "0 1px 3px #00000008", border: `1px solid #e5e7eb`, borderTopColor: color }}>
    <div style={{ color: "#9ca3af", fontSize: 12, marginBottom: 8 }}>{label}</div>
    <div style={{ fontSize: 26, fontWeight: 900, color: "#111827", lineHeight: 1 }}>{value}</div>
    {sub && <div style={{ color: "#6b7280", fontSize: 12, marginTop: 8, lineHeight: 1.4 }}>{sub}</div>}
  </div>
);
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: "'Apple SD Gothic Neo', 'Malgun Gothic', sans-serif", background: "#f8fafc", minHeight: "100vh" }}>
      ...
      {/* ─── 헤더 ─── */}
      <div style={{ background: "#fff", borderBottom: "1px solid #e5e7eb", padding: "20px 32px 0" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          <div>
            <div style={{ fontSize: 11, color: "#9ca3af", letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 4 }}>
              Trace Corp · 멜라체리
            </div>
          ...
          <div style={{ textAlign: "right", paddingBottom: 4 }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 800, color: "#f59e0b" }}>월 순이익 2,000만원</div>
          </div>
        </div>
      ...
      <div style={{ padding: "28px 32px", maxWidth: 900, margin: "0 auto" }}>
        ...
        {tab === "현황" && (
          <div>
            {/* 목표 달성 배너 */}
            <div style={{ background: "#fff", borderRadius: 14, padding: "24px 28px", marginBottom: 24, border: "1px solid #e5e7eb", boxShadow: "0 1px 3px #00000008" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                <div>
                  <div style={{ fontSize: 12, color: "#9ca3af", marginBottom: 6 }}>월 순이익 목표 달성률</div>
                  <div style={{ display: "flex", alignItems: "baseline", gap: 12 }}>
                    <span style={{ fontSize: 42, fontWeight: 900, color: "#f59e0b", lineHeight: 1 }}>{pct}%</span>
                    <span style={{ fontSize: 14, color: "#9ca3af" }}>
                      {(monthlyEst / 10000).toFixed(0)}만원 / 2,000만원
                    </span>
                  </div>
                </div>
                <div style={{ background: "#fef2f2", borderRadius: 10, padding: "12px 18px", textAlign: "center" }}>
                  <div style={{ fontSize: 11, color: "#9ca3af", marginBottom: 4 }}>목표까지 부족</div>
                  <div style={{ fontSize: 22, fontWeight: 900, color: "#ef4444" }}>
                    -{(gap / 10000).toFixed(0)}만원
                  </div>
                  <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 4 }}>현재의 {(TARGET / monthlyEst).toFixed(1)}배 성장 필요</div>
                </div>
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 24 }}>
              ...
              <Card label="주간 평균 판매량" value={`${avgSales}개`} sub={`최고: 389개 (2월1주)`} color="#10b981" />
              <Card label="주간 평균 판매이익" value={`${(avgProfit/10000).toFixed(0)}만원`} sub={`월 환산 ${(monthlyEst/10000).toFixed(0)}만원`} color="#8b5cf6" />
              <Card label="목표 달성 필요 판매량" value={`주 ${neededWeekly}개`} sub={`현재 대비 ${(neededWeekly/avgSales).toFixed(1)}배`} color="#f59e0b" />
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              <div style={{ background: "#fff", borderRadius: 12, padding: "20px 16px", border: "1px solid #e5e7eb" }}>
                ...
                <ResponsiveContainer width="100%" height={160}>
                  <BarChart data={kpiData} barSize={34}>
              ...
              <div style={{ background: "#fff", borderRadius: 12, padding: "20px 16px", border: "1px solid #e5e7eb" }}>
                ...
                <ResponsiveContainer width="100%" height={160}>
                  <LineChart data={kpiData}>
                    <XAxis dataKey="week" tick={{ fill: "#9ca3af", fontSize: 11 }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fill: "#9ca3af", fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={v => (v/10000).toFixed(0)+"만"} />
                    <Tooltip content={<Tip />} />
        ...
        {tab === "판매추이" && (
          <div>
                ...
                <tbody>
                  {kpiData.map((r, i) => (
                    <tr key={i} style={{ borderBottom: "1px solid #f8fafc" }}>
                      ...
                      <td style={{ padding: "13px 18px", textAlign: "right", color: "#374151" }}>{(r.기여이익/10000).toFixed(1)}만</td>
            ...
            <div style={{ background: "#fff", borderRadius: 12, padding: "20px 16px", border: "1px solid #e5e7eb" }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#374151", marginBottom: 4 }}>기여이익 vs 판매이익 추이</div>
              <div style={{ display: "flex", gap: 20, marginBottom: 14 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 16, height: 3, background: "#8b5cf6", borderRadius: 2 }} /><span style={{ fontSize: 12, color: "#6b7280" }}>판매이익</span></div>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 16, height: 3, background: "#10b981", borderRadius: 2 }} /><span style={{ fontSize: 12, color: "#6b7280" }}>기여이익</span></div>
              </div>
        ...
        {tab === "전략" && (
          <div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>
              <div style={{ background: "#fff", borderRadius: 12, padding: "20px 22px", border: "1px solid #fee2e2" }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: "#dc2626", marginBottom: 16 }}>⚠️ 핵심 문제</div>
                {[
                  ["판매량 급변동", "주간 58~389개, 평균 209개 — 안정성 부족"],
                  ["목표 대비 20% 수준", "월 이익 ~400만원 → 목표의 1/5 수준"],
                  ["리뷰 전환율 낮음", "2월3주 리뷰 130개 투입 → 판매 58개 역효과"],
                ].map(([t, d], i, arr) => (
                  <div key={i} style={{ paddingBottom: i < arr.length-1 ? 12 : 0, marginBottom: i < arr.length-1 ? 12 : 0, borderBottom: i < arr.length-1 ? "1px solid #fef2f2" : "none" }}>
                    <div style={{ fontWeight: 600, fontSize: 13, color: "#111827", marginBottom: 3 }}>{t}</div>
                    <div style={{ fontSize: 12, color: "#6b7280", lineHeight: 1.5 }}>{d}</div>
                  </div>
                ))}
              </div>

              <div style={{ background: "#fff", borderRadius: 12, padding: "20px 22px", border: "1px solid #d1fae5" }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: "#059669", marginBottom: 16 }}>✅ 즉시 실행 액션</div>
                {[
                  ...
                  <div key={i} style={{ paddingBottom: i < arr.length-1 ? 12 : 0, marginBottom: i < arr.length-1 ? 12 : 0, borderBottom: i < arr.length-1 ? "1px solid #f0fdf4" : "none" }}>
                    <div style={{ fontWeight: 600, fontSize: 13, color: "#111827", marginBottom: 3 }}>{t}</div>
                    <div style={{ fontSize: 12, color: "#6b7280", lineHeight: 1.5 }}>{d}</div>
                  </div>
                ))}
            ...
            <div style={{ background: "#fff", borderRadius: 12, padding: "22px 24px", border: "1px solid #e5e7eb" }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: "#0f172a", marginBottom: 20 }}>🗓️ 월 2,000만 달성 로드맵</div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 20 }}>
                {[
                  ...
                  <div key={i} style={{ background: r.bg, borderRadius: 10, padding: "16px", borderTop: `3px solid ${r.color}` }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: r.color, marginBottom: 6 }}>{r.month}</div>
                    <div style={{ fontSize: 20, fontWeight: 900, color: "#0f172a", marginBottom: 4 }}>{r.goal}</div>
                    <div style={{ fontSize: 12, color: "#6b7280", marginBottom: 6 }}>{r.weekly}</div>
                    <div style={{ fontSize: 11, color: "#9ca3af" }}>{r.action}</div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ background: "#f8fafc", borderRadius: 8, padding: "14px 18px" }}>
                <div style={{ fontSize: 11, color: "#9ca3af", fontWeight: 600, marginBottom: 10 }}>성장 경로</div>
